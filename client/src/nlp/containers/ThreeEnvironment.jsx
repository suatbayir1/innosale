...
import { getFileSpecs, getFileList, getTeklifId, editPartInMongoDb, getPartProcessList } from "../../store/index";
...
class ThreeEnvironment extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            camera: EnvironmentService.createCamera(75, window.innerWidth / window.innerHeight, 0.1, 2000),
            renderer: EnvironmentService.createRenderer(window.devicePixelRatio || 1, window.innerWidth, window.innerHeight),
        }
    ...
    initMainGUI = (clusterData) => {
        ...
        this.setState({ gui: new GUI({ touchStyles: false }) }, () => {
            ...
            importFolder.add(this.mainGUISelected, 'Teklif ID', Object.keys(this.props.teklifIdList)).$widget.style.color = "black"
            importFolder.add(this.mainGUISelected, 'Import Button')
            importFolder.add(this.mainGUISelected, 'Export Button').enable(false)

            gui.add(this.mainGUISelected, 'Scene Type', Object.keys(this.state.sceneList)).$widget.style.color = "black"
            ...
            gui.onChange((event) => {
                ...
                if (event.property.startsWith("Cluster")) {
                    if (event.property.endsWith("Color")) {
                        ...
                        EnvironmentService.editObjecInSceneList(sceneList, camera, renderer, event.property, this.mainGUISelected, this.props.fileSpecs.cluster_indexes, clusterIndex, event.value)
                    }
                }
                    ...
                    const { sceneList, camera, renderer } = this.state
                    EnvironmentService.editObjecInSceneList(sceneList, camera, renderer, event.property, this.mainGUISelected)
                }
            })
        })
    }
    ...
    onImportButtonClick = async () => {
        ...
        console.log(this.props.teklifIdList[this.mainGUISelected['Teklif ID']])
        while (true) {
            if (this.props.fileSpecs.name === this.props.teklifIdList[this.mainGUISelected['Teklif ID']]) {
    ...
    saveToDatabase = async () => {
        if (this.Clusters ? this.Clusters.length > 0 : false) {
            await this.guiClusterFolder?.controllersRecursive().forEach(controller => { if (controller.$name.innerHTML === "Save to Database") controller.enable(false) })
            await this.guiClusterFolder?.foldersRecursive().forEach(folder => folder.close())
            ...
            var mesh_object = await EnvironmentService.findObjectFromGroupInSceneByUuid(
                this.state.sceneList.Mesh, 'MeshGroup', 'MeshObject'
            )
            var cluster_color_list = await Array.from(mesh_object.geometry.attributes.color.array)
            ...
            await this.guiClusterFolder?.controllersRecursive().forEach(controller => { if (controller.$name.innerHTML === "Save to Database") controller.enable(true) })
        }
    }
    ...
    resetGuiInterface = () => {
        ...
        for (let i = 0; i < cluster_names.length; i++) {
            ...
            folder.add(this.Clusters[i], process, this.props.partProcessList /*['Açınım kesme','Bükme','Çekme','Çevre kesme','Delik','Delme','Kamlı delme','Kamlı kesme']*/).$widget.style.color = "black"
        ...
        this.state.gui.controllersRecursive().forEach(value => {
            if (value.property.startsWith("Cluster ")) {
                ...
                value.setValue(this.mainGUISelected[value.property])
        })
    }
    ...
    initEnvironment = async () => {
        await this.setRendererDiv()
        const { sceneList, camera, renderer } = await this.state
        this.environmentService = await new EnvironmentService(sceneList, camera, renderer)
    ...
    onPointDblClick = (raycaster) => {
        ...
        const color = new THREE.Color(
            ...
            intersects[0].object.geometry.attributes.color.array[closestIndex * 3 + 1],
    ...
    onMeshDblClick = (raycaster) => {
        ...
        this.mainGUISelected.SelectedPointIndex = intersects[intersects.length - 1].faceIndex
        ...
        this.guiClusterFolder.foldersRecursive().forEach(folder => {
            ...
            var clusterIndex = this.props.fileSpecs.cluster_indexes[closestIntersect.faceIndex]

            if (folderName === `Cluster ${clusterIndex + 1}`)
                folder.open()
            else
                folder.close()
        })
